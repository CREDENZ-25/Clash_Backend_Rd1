...
// async function setDoubleDipLifeline(req, res) 
//   {
  ...
  try {
    ...
    if (!question.options || question.correct === undefined) {
      return res
        .status(500)
        .json({
          error: "'options' or 'correct' field is missing in the question",
        });
    }
    ...
    if (incorrectIndices.length < 2) {
      return res
        .status(500)
        .json({ error: "Not enough incorrect answers to apply 50-50" });
    }

    const shuffledIncorrectIndices = incorrectIndices.sort(
      () => 0.5 - Math.random()
    );
    const reducedOptions = [correct, shuffledIncorrectIndices[0]];
